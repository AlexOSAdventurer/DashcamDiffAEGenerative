...
output_file_frames = "./video_val_3x256x256.npy"
...
def parseFile(file, json_file, i, mmap_frames_out, mmap_label_out):
    out, err = ffmpeg.input(file).output('pipe:', format='rawvideo', pix_fmt='rgb24').run(capture_stdout=True)
    video = torch.from_numpy(numpy.frombuffer(out, numpy.uint8).reshape(-1, output_size, output_size, 3))
