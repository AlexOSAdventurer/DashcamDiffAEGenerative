...
base_dir = "./"
dataset_path_train = base_dir + "video_train_256x256_latent_2.npy"
dataset_path_val = base_dir + "video_val_256x256_latent_2.npy"
new_dataset_path_train =  base_dir + "video_train_256x256_latent_diffusion.npy"
new_dataset_path_val =  base_dir + "video_val_256x256_latent_diffusion.npy"
max_epoch = 10
batch_size = 20
gpus = 2
device = 'cuda'
...
def infer_thread(rank, input_queue, output_queue):
    with torch.no_grad():
        dev = f"cuda:{rank}"
        autoencoder_model = diffusion_model.DiffusionModel_load_pretrained()
        ...
        while True:
            ...
            data = torch.reshape(data, (-1, 6, 64, 64))
            data = data.to(dev)
            z_sem = autoencoder_model.create_semantic(data, False)
            ...
            result = torch.reshape(z_sem.to('cpu'), (number_of_sequences, sequence_length, 512))
            output_queue.put((i, result))
            print(f"Inferred {i}")
            sys.stdout.flush()
            del i, data, number_of_sequences, sequence_length

def mmap_thread(input_queue, mmap_out):
    i = 0
...
def convertData(dataset, new_path):
    ...
    output_memmap = numpy.lib.format.open_memmap(new_path, dtype=numpy.float, shape=(len(dataset), dataset.sequence_length, 512), mode='w+')
    ...
    for rank in range(gpus):
        ...
        p.start()
        processes.append(p)
    mmap_process = mp.Process(target=mmap_thread, args=(mmap_queue, output_memmap))
    mmap_process.start()
